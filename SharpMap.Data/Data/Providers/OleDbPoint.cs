...
namespace SharpMap.Data.Providers
{
    ...
    public class OleDbPoint : PreparedGeometryProvider
    {
                        ...
                        if (dr != null && dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                if (!(dr.IsDBNull(0) || dr.IsDBNull(1)))
                                {
                                    ...
                                    yield return factory.CreatePoint(c);
                                }
                            }
                        }
                    }
        ...
        public override IGeometry GetGeometryByOid(object oid)
        {
            using (var conn = new OleDbConnection(ConnectionString))
            {
                ...
                var strSQL = "SELECT " + XColumn + ", " + YColumn + " FROM " + Table + " WHERE " + ObjectIdColumn +
                                "=" + string.Format(Map.NumberFormatEnUs,"{0}", oid);
        ...
        public override IFeature GetFeatureByOid(object rowId)
        {
            using (var conn = new OleDbConnection(ConnectionString))
            {
                var strSQL = "SELECT * FROM " + Table + " WHERE " + ObjectIdColumn + "=" + string.Format(Map.NumberFormatEnUs, "{0}", rowId);
                conn.Open();
