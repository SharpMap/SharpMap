...
//            }
        ...
        public IEnumerable<IGeometry> GetGeometriesInView(Envelope bbox, CancellationToken? cancellationToken = null)
		{
			...
			var objectlist = GetOidsInView(bbox);
            ...
            using (Stream s = OpenShapefileStream())
            {
                using (BinaryReader br = new BinaryReader(s))
                {
                    using (DbaseReader DbaseFile = OpenDbfStream())
                    {
                        ...
                        {
                            var fdr = GetFeatureByOid(oid, table, br, DbaseFile);
            ...
            using (Stream s = OpenShapefileStream())
            {
                using (BinaryReader br = new BinaryReader(s))
                {
                    using (DbaseReader DbaseFile = OpenDbfStream())
                    {
                        foreach (var oid in oids)
                        {
                            result.Add(GetGeometryByOid(oid, br, DbaseFile));
                        }
		...
		/// <param name="ds"></param>
		/// <returns></returns>
        public void ExecuteIntersectionQuery(Envelope bbox, IFeatureCollectionSet ds, CancellationToken? cancellationToken=null)
        {
            // Do true intersection query
            if (DoTrueIntersectionQuery)
            {
                ExecuteIntersectionQuery(Factory.ToGeometry(bbox), ds, cancellationToken);
                return;
            }
        ...
        IEnumerable<object> IProvider.GetOidsInView(Envelope view, CancellationToken? cancellationToken = null)
        {
            foreach (var oid in GetOidsInView(view))
            {
                yield return oid;
            }
        }
		...
		/// <param name="bbox"></param>
		/// <returns></returns>
        public Collection<uint> GetOidsInView(Envelope bbox)
        {
            //Use the spatial index to get a list of features whose boundingbox intersects bbox
        ...
        public IGeometry GetGeometryByOid(object oid)
        {
            ...
            using (Stream s = OpenShapefileStream())
            {
                using (BinaryReader br = new BinaryReader(s))
                {
                    ...
                    {
                        geom = ReadGeometry((uint)oid, br, dbf);
                        dbf.Close();
                    }
                    br.Close();
                }
		...
		/// <param name="oid">Object ID</param>
		/// <returns>The geometry at the Id</returns>
        private IGeometry GetGeometryByOid(uint oid, BinaryReader br, DbaseReader dbf)
        {
            if (_useMemoryCache)
            {
                FeatureDataRow fdr;
                ...
                if (fdr == null)
                {
                    fdr = (FeatureDataRow)GetFeatureByOid(oid);
                }
        ...
        public virtual void ExecuteIntersectionQuery(IGeometry geom, IFeatureCollectionSet ds, CancellationToken? cancellationToken = null)
        {
        ...
        {
            return GetFeatureByOid((uint)rowId);
        }

        public IFeature GetFeatureByOid(uint rowId)
        {
            return GetFeatureByOid(rowId, null);
        }
		/// <summary>
        ...
        /// <returns>The feature data row</returns>
        public FeatureDataRow GetFeatureByOid(uint rowId, FeatureDataTable dt)
        {
            ...
            using (Stream s = OpenShapefileStream())
            {
                using (BinaryReader br = new BinaryReader(s))
                {
                    using (DbaseReader dbfReader = OpenDbfStream())
                    {
                        ...
                        ret = GetFeatureByOid(rowId, dt, br, dbfReader);
                        dbfReader.Close();
                    }
                    br.Close();
                }
        ...
        private FeatureDataRow GetFeatureByOid(uint rowid, FeatureDataTable dt, BinaryReader br, DbaseReader dbfFileReader)
        {
            ...
            if (dbfFileReader != null)
            {
                ...
                if (_useMemoryCache)
                {
                    ...
                    if (fdr == null)
                    {
                        fdr = dbfFileReader.GetFeatureByOid(rowid, dt);
                ...
                // GetFeatureByOid returns null if the record has deleted flag
                if (fdr == null)
                    return null;
