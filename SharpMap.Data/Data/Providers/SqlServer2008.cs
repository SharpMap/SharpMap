...
{   
    ...
    {   
       ...
       {   
           ...
           using (var conn = new SqlConnection(ConnectionString))   
           {   
               string strSQL = "SELECT g." + GeometryColumn + ".STAsBinary() FROM " + Table + " g WHERE " + ObjectIdColumn + "=:POid";   
               conn.Open();   
               using (var command = new SqlCommand(strSQL, conn))
               {
                   command.Parameters.Add(new SqlParameter("POid", oid));
                   using (SqlDataReader dr = command.ExecuteReader())   
                   {   
                       while (dr.Read())   
                       {   
       ...
       {   
           using (var conn = new SqlConnection(ConnectionString))   
           {   
               string strSQL = "select g.* , g." + GeometryColumn + ".STAsBinary() As sharpmap_tempgeometry from " + Table + " g WHERE " + ObjectIdColumn + "=:POid";   
