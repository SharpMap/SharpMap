...
namespace SharpMap.Data.Providers
{
    ...
    public class OleDbTwoPointLine : PreparedGeometryProvider
    {
        ...
        {
            var objectlist = new Collection<object>();
            using (var conn = new OleDbConnection(ConnectionString))
            {
                //open the connection
                conn.Open();
        ...
        public override IGeometry GetGeometryByOid(object oid)
        {
            using (var conn = new OleDbConnection(ConnectionString))
            {
                ...
                var strSQL = "SELECT " + XColumnBegin + ", " + YColumnBegin + ", " + XColumnEnd + ", " + YColumnEnd +
                             " FROM " + Table + " WHERE " + ObjectIdColumn +
                             "= ?;";// + oid.ToString(Map.NumberFormatEnUs);

                using (var command = new OleDbCommand(strSQL, conn))
                {
                    var p = command.Parameters.Add(oid);
                    Debug.Assert(command.Parameters[p].Value == oid);
                    using (var dr = command.ExecuteReader())
                    {
                        if (dr != null && dr.HasRows)
                        {
        ...
        public override IFeature GetFeatureByOid(object oid)
        {
            using (var conn = new OleDbConnection(ConnectionString))
            {
                var strSQL = "SELECT * FROM " + Table + " WHERE " + ObjectIdColumn + "= ?;";
                conn.Open();

                using (var adapter = new OleDbDataAdapter(strSQL, conn))
                {
                    var p = adapter.SelectCommand.Parameters.Add(oid);
                    Debug.Assert(adapter.SelectCommand.Parameters[p].Value == oid);
